...
app.get('/getUsers', (req, res) => {
	...
	connection.query('SELECT * FROM GRPN_User;', function (error, results, fields) {
		if (error) throw error;
		console.log('le resultat de la requête est : ', results);
...
app.delete('/deleteUser', (req, res) => {
	connection.query('DELETE FROM GRPN_User WHERE userId='+ req.body.userId +';', function (error, results, fields) {
		if (error) throw error;
		console.log('le resultat de la requête est : ', results);
		res.status(200).json(results);
	});
